...
A persistent Dark/Light radio switch in the shared header of both pages. A first visit
follows `prefers-color-scheme` until the visitor chooses; the choice persists locally
across both pages. Page chrome, code preview, swatches, copied values and brand artwork
switch together, each reading only its own scheme's emitted variables. The `lightVisible`
build flag is gone. The Windows Terminal download stays the dark scheme.

Browser acceptance covered both pages and both themes at 1280px, 760px and 375px:
persistence, system following, no-JavaScript rendering, copied values, focus and the
