...
import { INSTALL, LIGHT_NOTE, PITCH, UNRELEASED_NOTE } from '../content.js';
...
const chrome = (current: Page): string => `<a class="skip-link" href="#content">Skip to content</a>
  <header class="site-head">
    <a class="mark" href="${current === 'home' ? '#overview' : '/#overview'}" aria-label="Aion home"><img src="/icon.png" alt="" width="44" height="44"></a>
...
const footer = (): string => `<footer class="site-foot">
  <div class="footer-intro"><img src="/aion-wordmark.webp" alt="Aion" width="144" height="48"><span>Gold, teal, and room to focus.</span></div>
...
export function landing(flags: SiteFlags): string {
  ...
  return `${chrome('home')}<article class="landing" id="content">
    <section class="hero" id="overview">
      <h1><img class="hero-brand" src="/aion-lockup-horizontal.webp" alt="Aion" width="1440" height="480"></h1>
...
export function palette(flags: SiteFlags): string {
  ...
  const sections = all.map((group) => `<section class="palette-group" id="${group.id}" aria-labelledby="${group.id}-title">
    <div class="section-heading"><div><h2 id="${group.id}-title">${group.title}</h2><p>${escapeHtml(group.description)}</p></div>${copyColours(group.swatches, flags, `Copy ${group.title.toLowerCase()}`)}</div>
    ${group.id === 'terminal' ? `<h3 class="set-label">Standard <span>Slots 0–7</span></h3>${swatchGrid(group.swatches.slice(0, 8), flags)}<h3 class="set-label">Bright <span>Slots 8–15</span></h3>${swatchGrid(group.swatches.slice(8), flags)}
      <p class="terminal-note">Use Background and Text from Foundations for the terminal’s default colours. ANSI black (slot 0) is intended as a background; applications using it as foreground text may be hard to read.</p>
      ${link('/#install', 'Get the Windows Terminal theme')}` : swatchGrid(group.swatches, flags)}
  </section>`).join('');
